---
title: "metoda walidacji krzyżowej na danych makroekonomicznych"
...
1.Gęstość zaludnienia (ilość ludzi na $1 km^2$) 'slaskie_gest', `warmmaz_gest`

2.Liczba ludności (pomocnicza zmienna do przeliczania wskaźników) `slaskie_ludnosc`, `warmmaz_ludnosc`

3.Liczba lokali mieszkalnych sprzedanych w ramach transakcji rynkowych `slaskie_Mieszkania`, `warmmaz_Mieszkania` (pozwolę sobie przekształcić zmienną do postaci wskaźnika na mieszkańca)

4.Liczba obiektów sportowych ogółem `slaskie_Sport`, `warmmaz_Sport` (pozwolę sobie przekształcić zmienną do postaci wskaźnika na mieszkańca)

5.Przeciętne miesięczne wynagrodzenie brutto `slaskie_Wynagrodzenia`, `warmmaz_Wynagrodzenia`

# Przygotowanie i wczytanie zmiennych

Funkcja do wczytania pliku, usuniecia niepotrzebnej kolumny i zmianie nazwy jednej kolumny:

```{r przygot, warning=FALSE, message=FALSE}
...
```{r tpwarocz, message=FALSE, warning=FALSE}
library(moments)
...
Wyniki testów wskazują, że dla gęstości zaludnienia oraz wynagrodzeń występują istotne różnice między województwami dla praktycznie każdej cechy rozkładu. Za to dla zmiennych Mieszkania, Obiekty Sportowe test nie wykazał istotnie statystycznych różnic i sugeruje że mamy większe podobieństwo ich rozkładów dla badanych regionów.
...
Porównanie wartości średnich wykazuje niezgodność w wynikach statystyk, jednak wnioski pozostają takie same, jeśli chodzi o różnice między województwami. Już tu możemy zobaczyć, że rozkład naszych danych nie jest idealnie normalny a 21 i 36 to zbyt mała liczba obserwacji, by testy były odporne na brak normalności. Znaczącą różnicę możemy zauważyć przy badaniu wariancji. Z klasycznego testu F wnioskujemy, że wariancje między województwami, niezależnie od zmiennej, są istotnie różne. Dowodzi to, że testy permutacyjne są tu lepszym wyborem.
...
```{r mackor, message=FALSE, warning=FALSE}
library(corrplot)
corrplot(cor(slaskie))
corrplot(cor(warmmaz))
```

Dla województwa Śląskiego: \* zmienne objaśniające: Gęstość, Ludność, Mieszkania \* zmianna objaśniana: Sport

Dla województwa Warmińsko - Mazurskiego: \* zmienne objaśniające: Sport, Wynagrodzenia \* zmienna objaśniana: Ludność

# Metoda k-fold dla województwa Śląskiego

k=9 (36 powiatów, więc po 4 w jednej grupie)

```{r kfoldslaskie, message=FALSE, warning=FALSE}
...
# Metoda k-fold dla województwa Warmińsko-Mazurskiego

k=7 (21 powiatów, więc po 3 w jednej grupie)

```{r kfoldwarmaz, message=FALSE, warning=FALSE}
